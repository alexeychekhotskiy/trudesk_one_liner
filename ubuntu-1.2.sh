...
echo -e "${RED}==========================================================================${NC}";
echo -e "version 1.2.10 - Community Edition - Copyright (C) 2014-2023 Trudesk Inc.";
echo -e "";
...
[yY]*)
  ...
  wget https://github.com/mikefarah/yq/releases/download/v4.20.2/yq_linux_amd64 -O /usr/bin/yq > /dev/null 2>&1
...
*)
  ...
  dpkg -i mongodb-org-database-tools-extra_5.0.6_amd64.deb > /dev/null;
  rm -rf mongodb-org-database-tools-extra_5.0.6_amd64.deb
  stop_spinner $?
  ;;
esac
...
cd /etc/
git clone http://github.com/jaywaytravel/helpdesk > /dev/null 2>&1;
cd /etc/trudesk
...
git checkout v1.2.10 > /dev/null 2>&1;
